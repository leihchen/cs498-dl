...
class YoloLoss(nn.Module):
    ...
    def get_regression_loss(self, box_pred_response, box_target_response):   
        ...
        return (
            ...
            F.mse_loss(torch.sqrt(box_pred_response[:, 2:]), torch.sqrt(box_target_response[:, 2:]), reduction='sum')
        )
    ...
    def find_best_iou_boxes(self, box_target, box_pred):
        ...
        ##### CODE #####
        S = self.S
        box_target_iou = torch.zeros(box_target.shape).type(torch.ByteTensor).to(self.device)
    ...
    def forward(self, pred_tensor,target_tensor):
        ...
        contains_object_mask = contains_object_mask.unsqueeze(3).expand_as(target_tensor)

        no_object_mask = target_tensor[:,:,:,4] == 0
        no_object_mask = no_object_mask.unsqueeze(3).expand_as(target_tensor)
