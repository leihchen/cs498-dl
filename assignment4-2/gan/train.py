...
def train(D, G, D_solver, G_solver, discriminator_loss, generator_loss, show_every=250, 
    ...
    for epoch in range(num_epochs):
        ...
        for x, _ in train_loader:
            ...
            d_error = discriminator_loss(disc_real, disc_fake, device)
            d_error.backward()
            D_solver.step()
            ...
            fake_images = G(noise).reshape(batch_size, input_channels, img_size, img_size)
            disc_fake = D(fake_images)
            g_error = generator_loss(disc_fake, device)
            g_error.backward()
            G_solver.step()
            ##########       END      ##########
            ...
            if (iter_count % show_every == 0):
                ...
                show_images(imgs_numpy[0:16], color=input_channels!=1, path='p1/img')
                plt.show()
                print()
            iter_count += 1