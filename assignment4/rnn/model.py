...
class RNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, model_type="rnn", n_layers=1):
        ...
        if model_type == 'rnn':
            self.rnn_layer = nn.RNN(hidden_size, hidden_size, n_layers, dropout=0)
        elif model_type == 'gru':
            self.rnn_layer = nn.GRU(hidden_size, hidden_size, n_layers, dropout=0)
        elif model_type == 'lstm':
            self.rnn_layer = nn.LSTM(hidden_size, hidden_size, n_layers, dropout=0)
        else:
    ...
    def forward(self, input, hidden):
        ...
        rnn_result, hidden = self.rnn_layer(embedding.reshape(1, input.shape[0], -1), hidden)
        output = self.decoder(rnn_result)

        
        ##########       END      ##########
        
        
        return output.reshape(input.shape[0], -1), hidden
    ...
    def init_hidden(self, batch_size, device=None):
        ...
        if self.model_type == 'lstm':
            # patch for hidden data dimension inconsistency, use default zeros for lstm
            return None
